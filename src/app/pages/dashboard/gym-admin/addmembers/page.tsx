...
export default function AddMemberPage() {
  ...
  useEffect(() => {
    const subscription = form.watch((_, { name }) => {
      if (name === "mobile") setDuplicateError(null);
    });
  ...
  // Auto-update expiry & fees when plan / joining date / paid amount changes
  useEffect(() => {
    if (!selectedPlan) {
    ...
    form.setValue("pendingAmount", Math.max(0, planPrice - Number(paidAmount)));
  }, [selectedPlan, joiningDate, paidAmount]);

  // ── Camera ────────────────────────────────────────────────────────────────

  // Callback ref: attaches stream to video element once it mounts in DOM
              ...
              <CardContent className="space-y-6">
                ...
                {duplicateError && (
                  <Alert variant="destructive">
                    ...
                    <AlertDescription>
                      A member with mobile <strong>{duplicateError.mobile}</strong> already exists:{" "}
                      <strong>{duplicateError.name}</strong> (Status: {duplicateError.status}).{" "}
                      Please use a different number or update the existing member.
                    </AlertDescription>
                  </Alert>
                )}
