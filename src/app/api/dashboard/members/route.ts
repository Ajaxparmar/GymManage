...
// Helper — check if gym has WA enabled + configured
...
export async function POST(req: NextRequest) {
  ...
  const gymId = session.user.gymId;
  if (!gymId) return NextResponse.json({ error: "No gym found" }, { status: 400 });

  try {
    const body = await req.json();
    const {
      name, mobile, email, address, photo,
      joiningDate, expiryDate, planId,
      totalFees, paidAmount = 0, pendingAmount,
    } = body;
    ...
    try {
      ...
      if (waReady) {
        ...
        console.log("[POST /api/dashboard/members] WA not configured — skipping welcome message");
      }
    } catch (err) {
      console.warn("[POST /api/dashboard/members] Welcome WhatsApp failed (non-blocking):", err);
    }
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (updateData.mobile && updateData.mobile !== existing.mobile) {
      const duplicate = await prisma.student.findFirst({
        where: { gymId, mobile: normalizePhone(updateData.mobile), id: { not: id } },
      });
      if (duplicate) {
        return NextResponse.json(
          { error: "Mobile number already in use by another member" },
    ...
    Object.keys(finalUpdate).forEach(
      (key) => finalUpdate[key] === undefined && delete finalUpdate[key]
    );

    if (Object.keys(finalUpdate).length === 0) {
      return NextResponse.json({ success: true, message: "No changes to apply", member: existing });
    }
    ...
    if (changedFields.length > 0) {
      try {
        ...
        if (waReady) {
          ...
          console.log("[PATCH /api/dashboard/members] WA not configured — skipping notification");
        }
      } catch (err) {
        console.warn("[PATCH /api/dashboard/members] WhatsApp notification failed:", err);
      }
